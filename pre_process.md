...
### 2. 分类特征数字化
* **处理逻辑**：对 `性别` 字段进行二值化编码（女 $\rightarrow$ 0， 男 $\rightarrow$ 1），并去除字符串前后空格，对无法映射的值用训练集众数填充。
...
* **处理逻辑**：采用 **KNN（K近邻）多变量高级插补**（设置 $K=5$）。  
  **重要前提**：为保证距离计算的合理性，**先进行缩尾和标准化，再执行KNN插补**，避免不同量纲特征主导邻近样本的选取。
* **包含特征**：
...
### 2. 异常值可视化及处理前后对比
为直观观测异常值处理对数据分布的影响，绘制了部分核心特征在**缩尾处理前后**的箱线对比图：

![处理前后箱线对比图](pictures/before_after_boxplot.png)  
*注：左箱为原始分布，右箱为缩尾后分布。可见极端离群点被收拢至 $99.5\%$ 分位数以内，但同时保留了高位的病理信号。*

### 3. 异常值处理策略：分位数缩尾（Winsorization）
* **处理逻辑**：摒弃传统的 $|x-\mu|>3\sigma$ 删除法，采用 **$0.5\%$ 与 $99.5\%$ 的分位数缩尾（盖帽法）**。针对连续型生理特征（排除性别和年龄），将低于训练集 $0.5\%$ 分位数和高于 $99.5\%$ 分位数的值，强制替换为对应的边界临界值。
* **保留病理信号**：极高的指标依然维持在 $99.5\%$ 分位数的高位，确保模型依然能有效识别出该样本的“极度异常”高危特征，不会丢失重度病患样本的信息。
